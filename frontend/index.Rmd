...
max_price <- quantile(PRICES$Price, .35)
product_names <- PRICES[
    (PRICES$ProductName %in% available_nvidia_chips) &
    (PRICES$Price <= max_price),
    "ProductName"
] |> unique()
plot <- plot_multiple_prices(product_names, "Greens")
best_prices <- price_by_date(product_names)[, c("Dia", "Nome", "Loja", "Preço")]
table_right <- DT::datatable(best_prices, style="bootstrap4", options=opts)
...
plot <- plot_multiple_prices(product_names, "Reds")
best_prices <- price_by_date(product_names)[, c("Dia", "Nome", "Loja", "Preço")]
table_right <- DT::datatable(best_prices, style="bootstrap4", options=opts)
...
plot <- plot_multiple_prices(product_names, "Blues")
best_prices <- price_by_date(product_names)[, c("Dia", "Nome", "Loja", "Preço")]
table_right <- DT::datatable(best_prices, style="bootstrap4", options=opts)
